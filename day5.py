...
## HOW TO START
List 3-5 steps a new developer should follow.
...
# ✅ NEW — Save report as markdown file
def save_report(sections, repo_url, repo_name):

    # Create output folder if it doesn't exist
    os.makedirs("output", exist_ok=True)

    # Create filename from repo name + today's date
    date = datetime.datetime.now().strftime("%Y-%m-%d")
    safe_name = repo_name.replace("/", "_")
    filename = f"output/{safe_name}_{date}.md"
    ...
    lines = []
    lines.append(f"# 🧭 Codebase Onboarding Guide\n")
...
def main():
    ...
    filename = save_report(sections, repo_url, repo_name)
    console.print(f"[bold green]✅ Report saved to: {filename}[/bold green]")
    console.print(f"[dim]Open it in VS Code or any markdown viewer![/dim]")
